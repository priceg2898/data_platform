...
def fetch_active_users():
    cur.execute("SELECT id, name, email, age, country, account_type FROM users ORDER BY id DESC LIMIT %s", (ACTIVE_WINDOW,))
    rows = cur.fetchall()
    users = []
    for r in rows:
        users.append({
...
def fetch_active_products():
    cur.execute("SELECT id, name, category, brand, price, sku FROM products ORDER BY id DESC LIMIT %s", (ACTIVE_WINDOW,))
    rows = cur.fetchall()
    products = []
    for r in rows:
        products.append({
...
def generate_user_event(user):
    ...
    return {
        ...
        "page": f"/products/{random.randint(1, 100)}" if event_type == "page_view" else None,
...
try:
    while True:
        ...
        if now - last_db_poll > DB_POLL_INTERVAL or not active_users or not active_products:
            active_users = fetch_active_users()
            active_products = fetch_active_products()
            last_db_poll = now
            print(f"Polled DB: {len(active_users)} users, {len(active_products)} products")
        ...
        if active_users and active_products:
            ...
            # Purchase events
            producer.send('purchase-events', generate_purchase_event(active_users, active_products))
